...
function O(n, o, c) {
  ...
  ;(e.className = o),
    ...
    e.appendChild(
      Object.assign(document.createElement('div'), { className: 'message', innerHTML: C(c) }),
    )
  const t = m.value
  e.setAttribute('data-matches', String(c.includes(t))), l.prepend(e)
}
let u = !1
      ...
      r.addEventListener('open', () => {
        ...
        r.addEventListener('message', t => {
          ...
          if (typeof i == 'object' && 't' in i && i.t === 'New' && 'entry' in i) {
            ...
            let y = S
            if (g !== void 0)
              for (const [E, w] of Object.entries(g)) y += ` ${E}=${JSON.stringify(w)}`
            O(b, L, y)
          }
        }),
